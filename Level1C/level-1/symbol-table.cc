...
void Symbol_Table ::generate_code(ostream &file_buffer){
	if(scope ==global && variable_table.size()){
		file_buffer <<GLOB_SPACE<<".data\n";
		list<Symbol_Table_Entry* > ::iterator it ;
		for(it = variable_table.begin();it!= variable_table.end();it++){
			file_buffer<<(*it)->get_variable_name()<<GLOB_VAR_SPACE<<".word 0\n";
		}
	}
} 
int Symbol_Table:: get_position(string var){
		int i =0;
		list<Symbol_Table_Entry* > :: iterator it;
		for(it = variable_table.begin();it!= variable_table.end();it++){
			if((*it)->get_variable_name() == var){
				return i;
			}
			i+=4;
		}
...
int Symbol_Table::get_symbol_table_size(){
