...
void Program::generate_code(){
	command_options.create_output_buffer();
	ostream & output_buffer = command_options.get_output_buffer();
	
	// Print the global symbol table here
	global_symbol_table.generate_code(output_buffer);	
	output_buffer <<GLOB_SPACE<<".text 			# The .text assembler directive indicates\n";
	output_buffer <<GLOB_SPACE<<".globl main		# The following is the code (as oppose to data)\n";
	
	Procedure * main = get_main_procedure(command_options.get_output_buffer());
    main->generate_code(output_buffer);
		
	// Now start printing the main here
}
